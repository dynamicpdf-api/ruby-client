...
class FormflattenAndremoveSamplesTest < Minitest::Test
  ...
  def test_PdfInputFilePath_Field_Pdfoutput
    ...
    if response.is_successful
      File.open("#{OUTPUT_PATH}FilePathField_PdfOutput.pdf", 'wb') { |file| file.write(response.content) }
    end

    File.open("#{OUTPUT_PATH}FilePathField_JsonOutput.json", 'w') { |file| file.write(pdf.get_instructions_json) }
  ...
  def test_PdfInputUsingCloudRoot_FormflattenFieldremove_Pdfoutput
    ...
    if response.is_successful
      File.open("#{OUTPUT_PATH}CloudRootFieldRemove_PdfOutput.pdf", 'wb') { |file| file.write(response.content) }
    end

    File.open("#{OUTPUT_PATH}CloudRootFieldRemove_JsonOutput.json", 'w') { |file| file.write(pdf.get_instructions_json) }
  ...
  def test_PdfInputUsingStream_FormflattenAllFields_Pdfoutput
    ...
    if response.is_successful
      File.open("#{OUTPUT_PATH}StreamAllFields_PdfOutput.pdf", 'wb') { |file| file.write(response.content) }
    end

    File.open("#{OUTPUT_PATH}StreamAllFields_JsonOutput.json", 'w') do |file|
  ...
  def test_PdfInputUsingFilePath_RetainSignature_Pdfoutput
    ...
    if response.is_successful
      File.open("#{OUTPUT_PATH}FilePathRetainSignature_PdfOutput.pdf", 'wb') { |file| file.write(response.content) }
    end

    File.open("#{OUTPUT_PATH}FilePathRetainSignature_JsonOutput.json", 'w') { |file| file.write(pdf.get_instructions_json) }
