...
class FormFillingSamplesTest < Minitest::Test
  ...
  def test_PdfInputFilePath_PdfOutput
    ...
    if response.is_successful
      File.open("#{OUTPUT_PATH}PdfInputFilePath_PdfOutput.pdf", 'wb') { |file| file.write(response.content) }
    end

    File.open("#{OUTPUT_PATH}PdfInputFilePath_JsonOutput.json", 'w') { |file| file.write(pdf.get_instructions_json) }
